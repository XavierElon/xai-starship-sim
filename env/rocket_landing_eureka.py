...
class RocketLander(MujocoEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Check if the reward function is provided and is callable
        if reward_function is None or not callable(reward_function):
            raise ValueError("A valid reward function must be provided and must be callable.")
        
        # Test the provided reward function
        test_reward, test_done = reward_function(self)
        if not isinstance(test_reward, (int, float)) or not isinstance(test_done, bool):
            raise ValueError("The provided reward function must return a tuple (reward, done) where reward is a number and done is a boolean.")

        self.reward_function = reward_function  # Store the provided reward function
    ...
    def _compute_reward(self):
        return self.reward_function(self)  # Call the provided reward function
