...
def build_env(rocket_design, resolution, curriculum_height=None, max_episode_steps=1000):
    """Create a high-resolution evaluation environment."""
    os.chdir(ROOT_DIR)
...
def build_actor(env, hidden_sizes, activation, scale_mapping, scale_lb, device):
    ...
    # Also build qvalue to match nn.ModuleList structure from training
    qvalue_net = MLP(
        num_cells=hidden_sizes,
        out_features=1,
        activation_class=act_cls,
    ...
    # Initialize weights with a fake forward pass
...
def run_eval_episode(env, actor, device, max_steps=1000):
    """Run a single deterministic evaluation episode, collecting frames."""
    frames = []
    ...
    # Infer outcome from final state (crash_report not available in rollout)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate SAC checkpoint with high-quality video")
    parser.add_argument("--checkpoint", type=str, required=True, help="Path to checkpoint .pt file")
    parser.add_argument("--resolution", type=int, default=720, help="Video resolution (default: 720)")
    parser.add_argument("--fps", type=int, default=40, help="Video FPS (default: 40, matches env)")
    ...
    # Try to pull network config from checkpoint
    ...
    # Checkpoint name for file naming
